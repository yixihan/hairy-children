<template>
  <n-card>
    ...
    <history-adopt-item v-for="item in list" :key="item.id" :item="item"></history-adopt-item>
  </n-card>
</template>
<script>
import { defineComponent, reactive, toRefs } from 'vue'
import { NCard, NBreadcrumb, NBreadcrumbItem } from 'naive-ui'
import { getUserInfo, getAllAdoptionNotices } from '../../api'
import { getData } from '../../utils/tools'
import historyAdoptItem from '../../components/history-adopt-item.vue'
...
export default defineComponent({
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      ...
      getAllAdoptionNotices({ userId: getData('userInfo').userId }).then((res) => {
